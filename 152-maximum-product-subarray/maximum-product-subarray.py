...
# Optimized solution >> There are three edges cases. 1) All +ve : Just multply everything. 2) Mixed -ve +ve : If the current number is +ve, the max is the max of previous * current. But if current number is -ve, the max is the min (because this would have the largest absolute value) of previous multiplied by the current number. 3) There is a ZERO : If zero exists, just reset the dp values to 1, so it wont affect the next number's decision.
# Time complexity >> O(n)
# Space complexity >> O(1)